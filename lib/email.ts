...
export async function sendQuoteReceivedEmail(quote: QuoteRequestRow) {
  ...
  await send(resend, {
    ...
    subject: `We've got your quote request \u2014 Order #${quote.order_number} \u2014 Royal Rollers`,
    text: [
      `Hi ${quote.contact_name},`,
      "",
      `Order #: ${formatOrderNumber(quote.order_number)}`,
      "",
...
export async function sendOwnerAlertEmail(quote: QuoteRequestRow) {
  ...
  await send(resend, {
    ...
    subject: `New quote request #${quote.order_number}: ${quote.contact_name} (${serviceLabel})`,
    text: [
      `Order #: ${formatOrderNumber(quote.order_number)}`,
      "",
...
export async function sendQuoteReadyEmail(quote: QuoteRequestRow, quoteAmountCents: number) {
  ...
  await send(resend, {
    from: FROM_ADDRESS,
    to: quote.contact_email,
    subject: `Your Royal Rollers quote is ready \u2014 Order #${quote.order_number}`,
    text: [
      `Hi ${quote.contact_name},`,
      "",
      `Order #: ${formatOrderNumber(quote.order_number)}`,
...
export async function sendBalanceChargeFailedEmail(params: {
  ...
  await send(resend, {
    ...
    subject: `We couldn't charge your card — Order #${params.orderNumber}`,
    text: [
      `Hi ${params.contactName},`,
      "",
      `Order #: ${formatOrderNumber(params.orderNumber)}`,
      "",
...
export async function sendBalanceChargeFailedOwnerAlertEmail(params: {
  ...
  await send(resend, {
    from: FROM_ADDRESS,
    to: OWNER_ALERT_ADDRESS,
    subject: `Balance charge failed: ${params.contactName} (Order #${params.orderNumber})`,
    text: [
      `Order #: ${formatOrderNumber(params.orderNumber)}`,
      "",
